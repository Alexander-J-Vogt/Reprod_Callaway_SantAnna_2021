...
## 4.0 Preparation of probabilities & vectors ----------------------------------
  ...
  for (i in seq_along(grouplist)) {
    ...
    # size of the unique oversvation in the panel dataset.
    weights[i, 1] <- g
  ...
  # This data frame is abbreviated from now on as Prob-ATTd-df.
...
## 4.1 Simple Weighted Average of ATT(g,t) -------------------------------------
  ...
  if (method == "simple_att") {
    ...
    # The simple-weighted ATT is a measure to evaluate the overall ATE of the 
    # treatment and might be a good first step to evaluate the treatment before 
    # continuing with less aggregated ATE. In order to calculate the measure,
    ...
    # Save the overall ATT (NO parial effects)
