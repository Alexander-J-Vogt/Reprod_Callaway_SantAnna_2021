---
title: "Difference-in-Difference with Various Treatment Timings and Multiple Periods"
output:
  html_document:
    df_print: paged
...
In this section, the original idea of the dynamic TWFE with staggered adoption is repeated with an accompanying explanation. Furthermore, the challenges of the dynamic TWFE are discussed as well as how the DiD design of CS (2021) solves these challenges.

The basic idea of a dynamic TWFE is to evaluate how the treatment effect of a policy evolves over several periods after its introduction. The basic dynamic TWFE can be extended for testing the PTA by allowing to include pre-treatment periods and let them interact with the treatment dummy. Lastly, the TWFE are included in order to capture everything what is affecting each unit equally over time and all the time-invariant characteristics of each unit.
...
The second problem is the so-called “negative weight problem”, which is discussed by Chaisemartin & D’Haultfoeuille (2020). This problem arises in the case of multiple periods and multiple groups when an “already-treated” group is used as a control group in one of the dynamic TWFE estimators. For a better understanding, recall the aforementioned example on the composition of the dynamic TWFE estimator. In this example, the “not-yet-treated” and “never-treated” group were used as a control group for the TWFE-DiD-estimator, which are both valid control groups for the DiD design as both groups resemble a theoretical potential outcome of an untreated group (Callaway, 2023). On the otherside, the “already-treated” group is not a valid representation of the theoretical potential outcome of an untreated group since this group received a treatment at an earlier time point (Callaway, 2023). Now, if the “already-treated” group has dynamic treatment effects, i.e., the treatment effects do not stay constant over the post-treatment periods, then the sub-DiD estimator has negative weights attached (Callaway, 2023). Thus, the dynamic TWFE coefficient is biased and does not allow a reliable causal interpretation of the treatment (Callaway, 2023).

Overall, it can be concluded that the dynamic TWFE regression leads to biased estimators for each period in the case of dynamic heterogeneous effects, due to the two above mentioned reasons. CS (2021) can solve both of these problems by their two-step procedure to calculate the aggregated ATE by first calculating the $ATT(g,t)$ with clearly defined control groups (i.e., “not-yet-treated” and “never-treated” groups) and then using the probability of belonging to a group to aggregate the $ATT(g,t)$. Therefore, the “negative weights problem” is avoided by using a transparent weighting process and the use of well-defined control groups.
...
# 1. Standardize variables & basic variables
# In this section, the arguments given to the function are re-defined in order
# to allow a universal usage of the function.
data_original <- data
...
for (g in grouplist) {
 ...
 # an implementation of BMisc::panel2cs2. see Callaway (2024)). The change
 ...
 # and IPW and has the advantage that it yields an identified estimator,
 ...
 # of Sant’Anna & Zhao (2020). Thereby, y1 is the required post-treatment outcome vector,
 # y0 is the pre-treatment outcome vector and D the treatment vector.
...
# 3. Aggregated ATT(g,t) according to CS (2021) ----
 ...
 # After having determined the ATT(g,t) for each group in each period, the
 # weights for aggregating the different ATT(g,t) need to be calculated.
 # These weights are equal to the probability of belonging to a treated group 
 # and are calculated in the following..
 # a) An empty dataset for saving the weights is created.
 weights <- as.data.frame(matrix(NA, nrow = length(grouplist), ncol = 3))
 colnames(weights) <- c("group", "size", "probs")
  
 # b) The weights of each group are calculated in this loop. seq_along
 # creates a number of vectors with length of grouplist, on which the loop iterates on.
 for (i in seq_along(grouplist)) {
  # Selecting the group for which the weight is calculated with the vector of unique groups
  g <- grouplist[i]

  weights[i, 1] <- g
  # The weights are determined by calculating the size of the subgroup g
  weights[i, 2] <- nrow(data[data$group == g & data$year == g, ])
  # and dividing it by the total size of unique observations in the panel dataset.
  weights[i, 3] <- weights[i, 2] / n_unique
 }
 
 # c) The probabilities of each group are getting merged with the ATT(g,t) by groups. 
 # This dataset is playing a key role for the calculation of the Agg. ATE.
...
if (method == "simple_att") {
 ...
 # all the weighted ATT(g,t) are summed up and this sum is than divided by kappa. 

 # This line selects all the relevant ATT(g,t) in the post-treatment period with 
 # index_post.
 ...
 # The simple-weighted ATE is calculated according to the description above.
...
if (method == "group_specific_att") {
 # 1) Partially Agg. ATE
 # The group-specific ATE is relevant for researchers if they want to
 ...
 # period from the data frame with all ATT(g,t).
 ...
 # The second step is to calculate the group-specific ATE by taking the
 # mean of all ATT(g,t) of a group g according to Sant'Anna (2022). In each 
 # loop iteration, the group-specific ATE is calculated for another group.
 for (i in seq_along(grouplist)) {
  g <- grouplist[i]
 ...
 # 2) Overall Agg. ATE
 # The overall ATE of the group-specific ATE is calculated by taking the sum of
 # the weighted partially Agg. ATE. This sum is divided by sum of all group
 # weights in order to normalize the overall Agg. ATE. Thus, the calculation
 # follows the same logic as the one of simple-weighted ATE.
...
if (method == "calendar_att") {
 ...
 # the ATE after a certain number of periods is (CS, 2021). The calendar time 
 # effects are calculated by taking the sum of all ATT(g,t) of one year with the 
 # corresponding weights of each group and divide by the sum of the weights used
 # in the this year.
   
 # Selecting the ATT(g,t) of each group in the post-treatment.
 aggte_ct <- attgt_probs_df[index_post,]

 # The calendar time effects are calculated by looping over all unique post-treatment
 # periods and, hence, these unique periods need to be determined.
 ...
 # In this loop, the calendar effects for each post-treatment period is calculated.
 for (i in seq_along(calendar_timelist)) {
  # First, the ATT(g,t) of the current period-iteration is selected. 
  calendar_time <- calendar_timelist[i]
  df <- aggte_ct[aggte_ct$year == calendar_time,]
  # Secondly, the calendar time effects are calculated by weighting the ATT(g,t) with the
  # corresponding weights in one year and divide it by the sum of all used weights 
  # in this period.
  cte_results[i, "time"] <- calendar_time
  cte_results[i, "coef"] <- round(sum(df$attgt * df$probs) / sum(df$probs), 4)
 }
...
if (method %in% c("unbalanced_eventstudy", "balanced_eventstudy")) {
 ...
 # unbalanced groups. The interest lays on the heterogeneity of ATE with respect 
 # to exposure to the treatment. In the first step, the event-time needs to be
 # calculated by taking the difference between the years and groups.
 attgt_et <- attgt_probs_df
 attgt_et$eventtime <- attgt_et$year - attgt_et$group

 # Than, all pre-treatment event times are excluded as the interest lays on the 
 # exposure to the treatment period.
...
if (method == "unbalanced_eventstudy") {
 ...
 # sum of all ATT(g,t) of an event-time e is taken and divided by the 
 # sum of weights of all groups in event-time e.

 # The calculation of the event study effect requires a variable with the
 # unique set of event-times. Than, the loop can be used to calculate the
 # event study effects for each event-time e.
 eventime_timelist <- unique(attgt_et$eventtime)

 # Creating an empty dataset to safe the event study effects of each event-time e
 et_results <- as.data.frame(matrix(NA, nrow = length(eventime_timelist), ncol = 2))
 colnames(et_results) <- c("time", "coef")
 
 # This loop calculates the event-study effects for each even-time.
 for (i in seq_along(eventime_timelist)) {
  ...
  et_results[i, "coef"] <- round(sum(df$attgt * df$probs) / sum(df$probs), 4)
 }
 ...
 # Save final event study effects
...
if (method == "balanced_eventstudy" & !is.null(balanced)) {
 ...
 # (B) Secondly, all the groups, which were treated at least for the certain
 # amount of periods, must be determined. Thus, the number of post-treatment 
 # periods is calculated by iterating over the vector with the unique treatment groups.
 ...
 att_per_group <- sapply(grouplist_et, function(g){
  ...
  # Than the number of post-treatment period is determined.
  n  <- nrow(df)
  n
 })
 ...
 # the required periods determined by "balanced_group". 
 groups_to_exclude <- c()
 for (i in seq_along(att_per_group$eventtime)) {
  # The groups with "balance_group + 1" period are excluded in the if-condition.
  if (att_per_group[i, 2] < balance_groups + 1) {
  #"groups_to_exclude" is containing the groups, which were not observed for required treatment length.
 ...
 for (i in seq_along(eventime_timelist)) {
  ...
  att_et_bal[i, "coef"]   <- round(sum(df$attgt * df$probs) / sum(df$probs), 4)
 }  

 # Only these event study effects are shown, which lay in the minimum requirement
...
calculate_att_se <- function(data = qwi,
 ...
 for (i in 1:iter) {
  ...
  # distribution is the following term of CS (2021):
  kappa <- (sqrt(5) + 1) / 2
  p <- kappa / sqrt(5)
  ...
  # of the data allows to estimate cluster standard errors on the unit-level.
  ...
  # list contain the distribution of every partial and overall ATE after
 ...
 # First, the lists, which contains the ATT must be collapsed and are converted 
 # into a data frame.
 b_res_partial <- map_dfr(b_res_partial, as.data.frame)
 b_res_overall <- unlist(b_res_overall)
 
 # Then, the standard error of the partial and overall Agg. ATE are calculated
 # from their respective distribution of ATE.
 se_partial <- apply(b_res_partial, 2, sd)
 se_overall <- round(sd(b_res_overall), 4)
 
 # After calculating the standard errors, the partial and overall Agg. ATE are
 ...
 # The results of the partial and overall Agg. ATE and the corresponding
...
Callaway, B. (2023). Introduction to DiD with Multiple Time Periods. Retrieved June 2, 2024, from https://bcallaway11.github.io/did/articles/multi-period-did.html
